...
class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    // --- Subjects ---
    fun insertSubject(name: String, teacher: String, room: String, color: String, startDate: String, endDate: String): Long {
    ...
    fun getAllClasses(): List<com.example.quanlylichhoc.utils.ClassItem> {
        ...
        val selectQuery = "SELECT c.*, s.$KEY_SUBJECT_NAME, s.$KEY_SUBJECT_TEACHER, s.$KEY_SUBJECT_COLOR, s.$KEY_SUBJECT_START, s.$KEY_SUBJECT_END FROM $TABLE_CLASSES c INNER JOIN $TABLE_SUBJECTS s ON c.$KEY_SUBJECT_ID = s.$KEY_ID"
    ...
    fun updateTask(id: String, title: String, desc: String, priority: String, deadline: String) {
        val db = this.writableDatabase
        val values = ContentValues().apply {
             put(KEY_TASK_TITLE, title)
             put(KEY_TASK_DESC, desc)
    ...
    fun getAllTasks(): List<com.example.quanlylichhoc.utils.TaskItem> {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                tasks.add(com.example.quanlylichhoc.utils.TaskItem(id, title, desc, priority, isCompleted, deadline))
            } while (cursor.moveToNext())
        }
        cursor.close()
        return tasks
    ...
    fun getAllSubjects(): List<SubjectItem> {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                subjects.add(SubjectItem(id, name, teacher, room, color, scheduleSummary, startDate, endDate))
            } while (cursor.moveToNext())
        }
        cursor.close()
        return subjects
    ...
    fun getSubjectById(id: String): SubjectItem? {
        ...
        if (cursor.moveToFirst()) {
            ...
            subject = SubjectItem(id, name, teacher, room, color, schedule, startDate, endDate)
        }
        cursor.close()
        return subject
    }
